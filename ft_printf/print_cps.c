...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/09 16:36:55 by hyunkim           #+#    #+#             */
/*   Updated: 2020/12/09 16:36:55 by hyunkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//원래는 '0'플래그와 같이 사용될 수없다.
{
	...
	//정밀도가 '.'이 존재하고 값이 0이고 data값이 NULL인경우만 빼고 나머지경우의 수는 전부출력
		write(1, &data, 1);
	(info->ret)++;
	i = -1;
	if (info->flag == 1)//'-'플래그 존재할 경우
		while (++i < info->width - 1)//문자 출력하고 폭의 수 -1 만큼 뒤에 공백출력
			write(1, &" ", 1);
	i = -1;
	while (++i < info->width - 1)//출력된 공백수 결과값에 더해주기
		(info->ret)++;
}
...
void	print_p(t_info *info, va_list ap)
{
	...
	data = va_arg(ap, unsigned long long);//unsigned int(32bit) 로 다시해보기
	i = -1;
	if (info->flag != 1)
...
void	print_s_1(t_info *info, int *size, char *data)//문자열은 '0'플래그와 같이 사용할 수없다.
{
	...
	else if (info->prec <= info->width && info->prec <= ft_strlen(data))//"%3.1s","abcd"=>[  a]
	//정밀도가 가장 작은경우에는 폭과 data의 상관관계는 중요하지않다.
	{
		size[0] = info->prec;
		size[1] = info->width - info->prec;//[1]에는 문자출력하고 남은 공간
	}
	else if (ft_strlen(data) <= info->width && ft_strlen(data) <= info->prec)//"%4.3s","ab"=>[  ab]
...
void	print_s(t_info *info, va_list ap)
{
	...
	if (data == NULL)
		data = ft_strdup("(null)");
	if (info->prec <= 0)//정밀도에 직접적으로 음수를 사용할수는 없고 *플래그를 사용하여
	//음수를 사용할수있고 음수이면 정밀도는 문자열의 길이와 똑같다
	...
	info->ret += size[0];
	i = -1;
	while (++i < size[1])//이거 대신에 info->ret += size[1] 실행해보기
		(info->ret)++;
	free(data);
	data = NULL;
